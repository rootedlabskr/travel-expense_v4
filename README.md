[travel_expense_tracker.html](https://github.com/user-attachments/files/28752221/travel_expense_tracker.html)
<!DOCTYPE html>
<html lang="ko">
<head>
<meta charset="UTF-8">
...
<div class="main">
    ...
    <div class="card" id="scanCard">
      ...
      <div id="multiItemArea" style="display:none">
        ...
        <div class="form-group" style="margin-top:10px">
          ...
          <div class="category-grid" id="scanCategoryGrid">
            <button class="cat-btn active" data-cat="식사" onclick="selectScanCategory(this)"><span class="cat-icon">🍽</span>식사</button>
            <button class="cat-btn" data-cat="쇼핑" onclick="selectScanCategory(this)"><span class="cat-icon">🛍</span>쇼핑</button>
            <button class="cat-btn" data-cat="교통" onclick="selectScanCategory(this)"><span class="cat-icon">🚌</span>교통</button>
            <button class="cat-btn" data-cat="숙박" onclick="selectScanCategory(this)"><span class="cat-icon">🏨</span>숙박</button>
            <button class="cat-btn" data-cat="관광" onclick="selectScanCategory(this)"><span class="cat-icon">🎡</span>관광</button>
            <button class="cat-btn" data-cat="의료" onclick="selectScanCategory(this)"><span class="cat-icon">💊</span>의료</button>
            <button class="cat-btn" data-cat="기타" onclick="selectScanCategory(this)"><span class="cat-icon">📦</span>기타</button>
          </div>
        </div>
        <div class="form-group">
    ...
    <div class="card" id="formCard">
      ...
      <div class="form-group">
        ...
        <div class="category-grid" id="categoryGrid">
          <button class="cat-btn active" data-cat="식사" onclick="selectCategory(this)"><span class="cat-icon">🍽</span>식사</button>
          <button class="cat-btn" data-cat="쇼핑" onclick="selectCategory(this)"><span class="cat-icon">🛍</span>쇼핑</button>
          <button class="cat-btn" data-cat="교통" onclick="selectCategory(this)"><span class="cat-icon">🚌</span>교통</button>
          <button class="cat-btn" data-cat="숙박" onclick="selectCategory(this)"><span class="cat-icon">🏨</span>숙박</button>
          <button class="cat-btn" data-cat="관광" onclick="selectCategory(this)"><span class="cat-icon">🎡</span>관광</button>
          <button class="cat-btn" data-cat="의료" onclick="selectCategory(this)"><span class="cat-icon">💊</span>의료</button>
          <button class="cat-btn" data-cat="기타" onclick="selectCategory(this)"><span class="cat-icon">📦</span>기타</button>
        </div>
      </div>
      <div class="form-row">
        <div class="form-group">
  ...
  <div class="page" id="page-list">
    <div class="filter-tabs">
      ...
      <div class="filter-tab" onclick="setFilter('식사')">🍽</div>
      <div class="filter-tab" onclick="setFilter('쇼핑')">🛍</div>
      <div class="filter-tab" onclick="setFilter('교통')">🚌</div>
      <div class="filter-tab" onclick="setFilter('숙박')">🏨</div>
      <div class="filter-tab" onclick="setFilter('관광')">🎡</div>
      <div class="filter-tab" onclick="setFilter('기타')">📦</div>
    </div>
    <div class="card">
      <div id="expenseList">
...
async function addAllSelectedItems() {
  ...
  const type = document.getElementById('scanTypeShared').classList.contains('active') ? 'shared' : 'personal';
  ...
  const category = getSelectedScanCategory();
  ...
  updateSummary();
  resetScanArea();
  showToast(`✅ ${count}개 항목 추가 완료! (${type === 'shared' ? '공동' : '개인'} · ${CATEGORIES[category]}${category})`);
}
...
function selectScanCategory(btn) {
  document.querySelectorAll('#scanCategoryGrid .cat-btn').forEach(b => b.classList.remove('active'));
  btn.classList.add('active');
}

function getSelectedCategory() {
  const active = document.querySelector('#categoryGrid .cat-btn.active');
  return active ? active.dataset.cat : '기타';
}

function getSelectedScanCategory() {
  const active = document.querySelector('#scanCategoryGrid .cat-btn.active');
  return active ? active.dataset.cat : '기타';
}
...
function addExpense() {
  ...
  saveToSheet(expense).then(ok => {
    showToast(ok ? '✅ 저장 완료! Google Sheets에 기록됐어요' : '⚠️ 앱에는 저장됐지만 Sheets 저장 실패');
  });
}
...
function editExpense(id) {
  ...
  // 폼에 기존 데이터 채우기
  setType(e.type);
  if (e.category) setCategoryActive(e.category);
...
function setFilter(f) {
  ...
  const filters = ['all','shared','personal','식사','쇼핑','교통','숙박','관광','기타'];
  const idx = filters.indexOf(f);
  if (idx >= 0) document.querySelectorAll('.filter-tab')[idx].classList.add('active');
  renderList();
}

function renderList() {
  let filtered = state.expenses;
  if (state.filter === 'shared') filtered = filtered.filter(e => e.type === 'shared');
  else if (state.filter === 'personal') filtered = filtered.filter(e => e.type === 'personal');
  else if (['식사','쇼핑','교통','숙박','관광','의료','기타'].includes(state.filter)) {
    filtered = filtered.filter(e => e.category === state.filter);
  }
  ...
  list.innerHTML = sorted.map(e => {
    const catIcon = CATEGORIES[e.category] || (e.type === 'shared' ? '🤝' : '👤');
    const catLabel = e.category || e.name || (e.type === 'shared' ? '공동' : '개인');
    return `
    <div class="expense-item">
      <div class="exp-badge ${e.type}" style="font-size:20px">${catIcon}</div>
      <div class="exp-info">
        <div class="exp-name">${catLabel}
          <span style="font-size:11px;color:var(--text3);font-weight:400;margin-left:4px">${e.type === 'shared' ? '🤝공동' : '👤개인'}</span>
        </div>
        <div class="exp-meta">
          ${e.date} · ${e.payer} 결제
          ${e.type === 'personal' ? ` · ${e.personalMember} 사용` : ''}
          ${e.memo ? ` · ${e.memo}` : ''}
          ${e.name && e.name !== e.category ? ` · ${e.name}` : ''}
        </div>
      </div>
      <div>
